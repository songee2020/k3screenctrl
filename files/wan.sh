...
WAN_STAT=`ifstatus lan`
WAN6_STAT=`ifstatus lan6`

# Internet connectivity
IPV4_ADDR=`echo $WAN_STAT | jsonfilter -e "@['ipv4-address'][0].address"`
IPV6_ADDR=`echo $WAN6_STAT | jsonfilter -e "@['ipv6-address'][0].address"`
...
if [ -z "$WAN_IFNAME" ]; then
    ...
    if [ -z "$WAN_IFNAME" ]; then
        WAN_IFNAME=`uci get network.lan.device` # eth0.2
    fi
fi
